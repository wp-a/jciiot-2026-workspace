...
## 当前组委会提交版本（2026-08-20）

代码入口位于 [`submission/`](submission/)，发布说明见
[`SUBMISSION_RELEASE_20260820.md`](SUBMISSION_RELEASE_20260820.md)。当前提交
严格使用 `physical_carry`：入口拒绝 `attachment` 和 `l1_floor_push`，失败时
fail closed。历史固定场景分数和旧路线只作为明确标注的研究对照，不作为本次
严格物理五关满分或官方评测声明。
