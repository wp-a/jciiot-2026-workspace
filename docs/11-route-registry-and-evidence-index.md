...
对当前工作区的 `.worktrees/` 和 `autoresearch/` 做了只读机器盘点：共发现
451 个 JSON 文件，其中 39 个包含 L2-L5 评分字段；没有任何一个候选同时满足
`attachment_calls=0`（或等价 attachment 计数为 0）和
`object_pose_writes=0`。因此，目前没有被旧工作树遗漏的 L2-L5 无 attachment
满分证据。这个盘点只能证明已保存材料中的状态，不能替代尚未记录的远程运行。
