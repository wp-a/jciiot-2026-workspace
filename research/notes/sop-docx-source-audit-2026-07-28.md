...
- 输出必须保存任务值、来源、证据定位和冲突，不能只输出无来源摘要；未完成校准前不虚构数值置信度。
...
后续实现和实验证明了五文档自动生成、固定 schema、官方实体交叉检查和
L2 冲突解析；结果见 `experiments/2026-07-28-sop-qwen3vl-generation.md` 和
`sop_generated/`。VLM 描述仍只作为辅助证据，不能单独证明图片语义准确性，
也不进入评分动作链。无冲突检查和无图片证据对照仍属于后续正式消融。
