...
- Controlled change: L4 only; calculate the alignment yaw from measured base,
  ...
  collision-checked yaw controller, then apply the existing read-only target
  verification.
...
- Safety boundary: no object qpos write, no attachment relative-state mutation,
  no protected-file modification, and no change to L1, L2, L3 or L5 routing.
